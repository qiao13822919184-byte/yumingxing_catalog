...
| 区域 | 可控制的内容 | 数据影响 |
|---|---|---|
| 产品 | 新增/编辑/删除、分类、顺序、主图/图库、双语描述、上下架、组件、自定义参数和详情块 | 保存后影响公开目录；未核验产品不能上架 |
| 分类 | 名称、双语描述、封面图、顺序、新增/删除 | 前台分类入口与筛选共用同一数据 |
| 页面设计 | 品牌文字、标题、强调色、列表列数、WhatsApp 号码 | 前台重新读取设置后生效 |
...
本机正式服务为 http://127.0.0.1:3000，数据库、图片和后台均在您自己的文件夹。GitHub 保存源码和初始静态素材，实际运营数据库、上传文件、初始密码与日志由 `.gitignore` 排除。

以后对客户上线需要自行准备公网服务器/域名及 HTTPS。用同一份代码构建并运行 Node.js 服务即可，保留可写数据目录。GitHub Pages 单独不能提供此后台。首次在另一台机器启动会生成它自己的随机管理员密码，不会上传或共享本机密码。
