...
export function CatalogClient({ products, settings = defaultSettings }: { products: Product[]; settings?: SiteSettings }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/catalog', { signal: controller.signal }).then(async r => { if (!r.ok) throw new Error(); return r.json(); }).then(data => { setLive(data.products); setConfig(data.settings); setLoading(false); }).catch(e => { if (e.name !== 'AbortError') { setFailed(true); setLoading(false); setNotice('The catalogue could not be loaded. Please refresh to try again. / 暂时无法加载产品目录，请刷新重试。'); } });
  ...
  return <main style={{ '--accent': config.accent, '--columns': config.columns } as CSSProperties}>
    ...
    <footer><a href="/" className="footer-brand">{config.brand}</a><p>{t('Stainless steel cutlery & kitchenware', '不锈钢餐具与厨房用具')}</p><a className="footer-whatsapp" href={`https://wa.me/${config.whatsapp}?text=${encodeURIComponent(t(`Hello ${config.brand}, I would like to know more about your products.`, `您好，我想进一步了解 ${config.brand} 的产品。`))}`} target="_blank" rel="noopener noreferrer">WhatsApp ↗</a><a href="/admin">{t('Catalogue management', '图册管理')}</a><span>© {new Date().getFullYear()} {config.brand}</span></footer>
